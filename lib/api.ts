...
// --- Response caches (shared across accounts, RAM-only) ---
//
// Flyers change weekly, so aggressive TTLs cost nothing in freshness and
// keep us a polite citizen of the unofficial Tjek API.
...
export async function searchDeals(query: string, limit = 20, countryId = "DK"): Promise<Offer[]> {
  const cacheKey = `${countryId}|${limit}|${query.toLowerCase()}`;
...
export async function getStoreOffers(dealerId: string, limit = 50): Promise<Offer[]> {
  return storeOffersCache.getOrFetch(`${dealerId}|${limit}`, async () => {
    const params = new URLSearchParams({
      dealer_id: dealerId,
      limit: String(limit),
    });
...
export async function listStores(countryId = "DK"): Promise<Dealer[]> {
  return dealersCache.getOrFetch(countryId, async () => {
    const params = new URLSearchParams({
      country_id: countryId,
      limit: "100",
    });
