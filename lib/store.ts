...
// --- RAM backend (HTTP mode) ---
//
// When a request carries an account context (remote MCP over HTTP), the
// profile lives ONLY in this per-instance map — nothing touches disk, ever.
// The map is a convenience cache (TTL ~24h, gone on cold start); durability
// is the client's job via export_profile/import_profile.

const RAM_TTL_MS = 24 * 60 * 60 * 1000;
...
const ramStores = new Map<string, RamEntry>();

function ramEntryFor(account: string): RamEntry | null {
  const entry = ramStores.get(account);
  if (!entry) return null;
  if (Date.now() - entry.updatedAt > RAM_TTL_MS) {
    ramStores.delete(account);
    return null;
  }
  return entry;
}

/** Whether an account has a live cached profile, and how old it is. */
export function profileCacheInfo(account: string): { exists: boolean; ageMs: number } {
  const entry = ramEntryFor(account);
  if (!entry) return { exists: false, ageMs: 0 };
  return { exists: true, ageMs: Date.now() - entry.updatedAt };
}

/** Test hook: wipe all in-RAM profiles. */
export function clearRamProfiles(): void {
  ramStores.clear();
}

// --- File I/O with mutex (stdio/local mode only) ---
...
async function loadRaw(): Promise<DataStore> {
  ...
  if (account !== null) {
    return ramEntryFor(account)?.data ?? emptyStore();
  }
...
async function saveRaw(data: DataStore): Promise<void> {
  const account = currentAccount();
  if (account !== null) {
    ramStores.set(account, { data, updatedAt: Date.now() });
    return;
  }
  const filePath = getStorePath();
